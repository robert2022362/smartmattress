function [interval_successrate_flag,T_interval_continuity_HR] = AM_TIME_1536(data,a,wavename)
%输入是原始数据
    ...
    %-----------30间隔--------black------------------
    [findpeaks_time_domain_max] = findpeaks_1536_2048(processed_data,1,1505);%找到black峰点
    ...
    %==========================================================================================================
    %------------30间隔----------red------------------
    [findpeaks_time_domain_min] = findpeaks_1536_2048(processed_data,0,1505);%找到red峰点
    ...
    if (a_matching_degree_max>=1300)&&(a_matching_degree_max<=1536)      %------90%↑-------
    ...
    elseif (a_matching_degree_max>=1100)&&(a_matching_degree_max<=1536)	%------80%↑-------
        interval_successrate_flag=1;                         %标注准确率等级
        heartrate_level_flag=1;         %找到的标志
        heartrate_time_domain_interval=max_flag_value(b_matching_degree_max);
    elseif (a_matching_degree_max>=1000)&&(a_matching_degree_max<=1536)  %------70%↑-------
        interval_successrate_flag=1;                         %标注准确率等级
        heartrate_level_flag=1;         %找到的标志
        heartrate_time_domain_interval=max_flag_value(b_matching_degree_max);
