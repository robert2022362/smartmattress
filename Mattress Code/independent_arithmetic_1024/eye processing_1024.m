...
% while ishandle(fig)
      for t=1:fix(length(datat)/512)
        t
        start=t*1024;
        data_iteration=datat(start:start+1023);
        ...
        hold on
        %-----------30间隔--------black------------------
        [findpeaks_time_domain_max] = findpeaks_1024(data_differenced,1);%找到black峰点
        peaks_count_max=length(findpeaks_time_domain_max);%找到的峰值个数
        for i=1:peaks_count_max
            %subplot(3,1,2);
        ...
        %==========================================================================================================
        %------------30间隔----------red------------------
        [findpeaks_time_domain_min] = findpeaks_1024(data_differenced,0);%找到red峰点
        peaks_count_min=length(findpeaks_time_domain_min);%找到的峰值个数，值可以为p-1
        for i=1:peaks_count_min
            %subplot(3,1,2);
       ...
       %disp("===============interval================")
        ...
        if a_matching_degree_max>867.6 %------90%-------
