function [ T_interval_continuity_HR ] = function_heartrate_250( data4,datalength)
%此处显示有关此函数的摘要
%   此处显示详细说
%本函数是用来计算180-250之间较低的心率的(争对BCG模拟器)
...
%     hold on
    %/////////////////////
    %-----------30间隔--------black------------------
    [findpeaks_time_domain_max] = findpeaks_22(processed_data,1,max_limit);%找到black峰点  1435是1536的数据长度 1947是2048的数据长度
...
%     end
    ...
    %==========================================================================================================
    %------------30间隔----------red------------------
    [findpeaks_time_domain_min] = findpeaks_22(processed_data,0,max_limit);%找到red峰点
...
%     end
    ...
    %=====================针对250心率=================
    if(a_max_flag(1)>=5)&&(max_statistics_black>20)%一个是连续数据一个是相同间隔数
        heartrate_level_flag=1;
        heartrate_time_domain_interval=round(max_flag_value(1));
    elseif(a_max_flag(2)>=5)&&(max_statistics_red>20)%一个是连续数据一个是
        heartrate_level_flag=1;
