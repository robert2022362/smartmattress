function [original_interval , statistics_array_2D , statistics_array , max_statistics , flag_2D , a_max_flag , max_flag_value , row_n1 , column_n1] = HR_calculate( new_findpeaks_time_domain_max_min )
%HR_calculate 此处显示有关此函数的摘要
    %-------------------------计算后一个点与前一个点的原始间隔--------------------
    peaks_count=length(new_findpeaks_time_domain_max_min);
    original_interval=[];
    for i=1:(peaks_count-1) %n个峰值的话，只要做n-1次减计算就可以了
        original_interval(1,i)=(new_findpeaks_time_domain_max_min(1,i+1)-new_findpeaks_time_domain_max_min(1,i));%这里减1是为了得到峰之间的间隔
    end
    %----------------------原始间隔分类--------二维数组-----------
    ...
    %//////////////////2021年4月21日/////
    max_statistics=max(statistics_array(1,:)); %得到每行自己的最大值，比如2，2这种连续又分段的情况下
    %//////////////////2021年4月21日/////
